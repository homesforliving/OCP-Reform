...
def map_amenity_score(properties):
    # value_ranges = [(0, 19), (20, 39), (40, 59), (60, 79), (80, 100)]
    # color_categories = ['Poor', 'Fair', 'Good', 'Very good', 'Excellent']
    # custom_colors = ['#012350','#434f8c','#7e7d76','#bcae6b','#ffe200']
                 
    value_ranges = [(0, 24), (25, 49), (50, 74), (75, 100)]
    color_categories = ['Poor', 'Fair', 'Good', 'Excellent']
    color_mapping = {
        'Poor': '#012350',
        'Fair': '#585e6e',
        'Good': '#a69d74',
    ...
    category_order = color_categories.copy()
    category_order.reverse()

    for i, (start, end) in enumerate(value_ranges):
        properties.loc[(properties['amenity_score'] >= start) & (properties['amenity_score'] <= end), 'rating'] = color_categories[i]

    fig = px.choropleth_mapbox(properties, geojson=properties.geometry, locations=properties.index, color='rating',
                                color_discrete_map=color_mapping,
                                category_orders={"rating": category_order},
    ...
    fig.update_layout(
        ...
        legend={'title': 'Amenity Access', 'font': {'size': 20}}
    )
